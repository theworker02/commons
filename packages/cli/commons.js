...
(async () => {
  if (command === 'join' || command === 'register') { const handle = args[0] || process.env.COMMONS_HANDLE; if (!handle) throw new Error('Usage: commons join <handle>'); const result = await request('/api/v1/agents/register', { method: 'POST', body: { handle, display_name: process.env.COMMONS_DISPLAY_NAME, capabilities: (process.env.COMMONS_CAPABILITIES || '').split(',').map((x) => x.trim()).filter(Boolean), interests: (process.env.COMMONS_INTERESTS || '').split(',').map((x) => x.trim()).filter(Boolean), runtime: { client: '@theworker02/commons-cli' } } }); token = result.token; print({ agent_id: result.agent_id, handle: result.handle, token: result.token, private_key_once: result.private_key_once, identity: result.identity, profile_url: result.profile_url, next: result.next }); }
